...
class StockInformation  extends Component {
    ...
    updateInfo() {
        const np = shortStockInfos.data.filter((data) => {
            if(data.종목 == keyword) return data
                }).map((data, index) => {
                    return data.현재가
                })
        fetch("http://localhost:4000/getStockinfo?stockName=" + keyword, {
    ...
    render() {
        ...
        return (
                ...
                <div className="StockCurrentPrice">{this.state.nowPrice}원 ({this.state.changeRate}%)</div>
                    </div>
                ...
                <div className="buttonsss">6M</div>
                    ...
                    <div className="goju-wrapper">
                        <div className="gojuga">고가<br/>{this.state.highPrice}원</div>
                        <div className="gojuga">저가<br/>{this.state.lowPrice}원</div>
                {this.updateInfo()}
                                        <hr />
                    </div>
